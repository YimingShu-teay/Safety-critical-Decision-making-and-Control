...
class IDM:
    ...
    def Clac_LandLF(self):
        ...
        if F_id_u is None:
            F_id_u = "UNoone"
            D_F_u = self.dyn.x - 0.0
        if F_id_d is None:
            F_id_d = "DNoone"
            D_F_d = self.dyn.x - 0.0
        if F_id_e is None:
            F_id_e = "ENoone"
            D_F_e = self.dyn.x - 0.0
...
################################################################################################################################    
    ...
    def threshold_demand_checking_l(self,rx,alpha_l):
        th_checking = (1 + alpha_l)*self.dyn.v + rx 
        return th_checking
    ...
    def Give_signal(self,H_pred,U_Line,D_Line,dt,a_ego,Bc,d0,E_center_y,U_center_y,D_center_y,change_distance,alpha_l,al,Bc_l,rx):
        L_id_u,  D_L_u, F_id_u,  D_F_u, L_id_d, D_L_d, F_id_d ,D_F_d, L_id_e, D_L_e, F_id_e, D_F_e = self.Clac_LandLF()
        ...
        th_checking_l = self.threshold_demand_checking_l(rx,alpha_l)
    ...
    #做出并返回direction
    def Decision_Making(self,U_Line,D_Line,Bc,d0,H_pred,dt,a_ego,desired_v,E_center_y,U_center_y,D_center_y,change_distance,alpha_l,epsilon,Bc_l,rx):    
        Safe_signal_L, Velocity_signal_L, Safe_signal_R, Velocity_signal_R, UDE_Signal = self.Give_signal(H_pred,U_Line,D_Line,dt,a_ego,Bc,d0,E_center_y,U_center_y,D_center_y,change_distance,alpha_l,epsilon,Bc_l,rx)   
    ...
    def Judge_Goal(self,U_Line,D_Line,goal_x,E_center_y,U_center_y,D_center_y,Bc,d0,H_pred,dt,a_ego,desired_v,change_distance,alpha_l,epsilon,Bc_l,rx):
        lane_now = self.Judge_Location(U_Line,D_Line)
        direction,Final_signal = self.Decision_Making(U_Line,D_Line,Bc,d0,H_pred,dt,a_ego,desired_v,E_center_y,U_center_y,D_center_y,change_distance,alpha_l,epsilon,Bc_l,rx)
        if lane_now == "U":
            if direction == "Stay":
    ...
    #single solve step
    def Solve_step_mpc(self,u_lim, x0, oa, od,T,NX,NU,R,Q,Rd,Qf,du_lim,MAX_ITER,DU_TH,v_lim,dt,U_Line,D_Line,goal_x,E_center_y,U_center_y,D_center_y,d_safe_x,d_safe_y,L,TARGET_SPEED,gamma_L,gamma_F,Rt,ROI,Bc,d0,H_pred,change_distance,gamma_ycbf,sigma,alpha_l,Bc_l,rx):
        goal,Final_signal= self.Judge_Goal(U_Line,D_Line,goal_x,E_center_y,U_center_y,D_center_y,Bc,d0,H_pred,dt,oa,TARGET_SPEED,change_distance,alpha_l,sigma,Bc_l,rx)
